var pedigreejs = (function (exports) {
  ...
  function nodeclick(node) {
    ...
    // Jianfeng modified, race attribute
    if ('race' in node) {
      $('#id_race_0').val(node.race);
    } else {
      $('#id_race_0').val('-');
    }

    // Jianfeng modified, ancestry attribute
    if ('ancestry' in node) {
      $('#id_ancestry_0').val(node.ancestry);
    } else {
      $('#id_ancestry_0').val('-');
    }

    // Jianfeng modified, intervention attribute
    if ('mastectomy' in node) {
      $('#id_mastectomy_0').val(node.mastectomy);
    } else {
      $('#id_mastectomy_0').val('-');
    }
    if ('hysterectomy' in node) {
      $('#id_hysterectomy_0').val(node.hysterectomy);
    } else {
      $('#id_hysterectomy_0').val('-');
    }
    if ('oophorectomy' in node) {
      $('#id_oophorectomy_0').val(node.oophorectomy);
    } else {
      $('#id_oophorectomy_0').val('-');
    }

    // Jianfeng modified, germline testing results attribute
    if ('APC' in node) {
      $('#id_APC_0').val(node.APC);
    } else {
      $('#id_APC_0').val('-');
    }
    if ('ATM' in node) {
      $('#id_ATM_0').val(node.ATM);
    } else {
      $('#id_ATM_0').val('-');
    }
    if ('BARD1' in node) {
      $('#id_BARD1_0').val(node.BARD1);
    } else {
      $('#id_BARD1_0').val('-');
    }
    if ('BMPR1A' in node) {
      $('#id_BMPR1A_0').val(node.BMPR1A);
    } else {
      $('#id_BMPR1A_0').val('-');
    }
  ...
  function save(opts) {
    ...
    // Jianfeng modified
    var ancestry = $('#id_ancestry_0').val();
    ...
    // Jianfeng modified
    var mastectomy = $('#id_mastectomy_0').val();
    if (mastectomy && mastectomy !== '') {
      person.mastectomy = mastectomy;
    } else {
      delete person.mastectomy;
    }

    var hysterectomy = $('#id_hysterectomy_0').val();
    if (hysterectomy && hysterectomy !== '') {
      person.hysterectomy = hysterectomy;
    } else {
      delete person.hysterectomy;
    }

    var oophorectomy = $('#id_oophorectomy_0').val();
    if (oophorectomy && oophorectomy !== '') {
      person.oophorectomy = oophorectomy;
    } else {
      delete person.oophorectomy;
    }

    // Jianfeng modified, germline testing results
    var APC = $('#id_APC_0').val();
    if (APC && APC !== '') {
      person.APC = APC;
    } else {
      delete person.APC;
    }
    var ATM = $('#id_ATM_0').val();
    if (ATM && ATM !== '') {
      person.ATM = ATM;
    } else {
      delete person.ATM;
    }
    var BARD1 = $('#id_BARD1_0').val();
    if (BARD1 && BARD1 !== '') {
      person.BARD1 = BARD1;
    } else {
      delete person.BARD1;
    }
    var BMPR1A = $('#id_BMPR1A_0').val();
    if (BMPR1A && BMPR1A !== '') {
      person.BMPR1A = BMPR1A;
    } else {
      delete person.BMPR1A;
    }
